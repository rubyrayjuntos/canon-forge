...
import { buildCharacterIdentityBlock } from '../utils/identityLock.js';
...
export function buildKeyframePrompt(
  ...
  return `${AESTHETIC_PROMPT_CORE}
    ...
    ${buildCharacterIdentityBlock(char, { hasFaceReference: Boolean(char.canonHeadshotUrl) })}
      ...
      try {
        const res = await fetch('/api/generate', {
          ...
          body: JSON.stringify({
            ...
            referenceImage: providerConfig.provider === 'gemini' ? canonHeadshotUrl : undefined,
            useInitImage: false,
          }),
        });
